...
`this` 在 JavaScript 这个大环境下始终存在指向不明确的问题，不利于静态分析和强类型检查。

### 本质区别

二者的区别，说来非常简单，但是又非常巨大：

* React Hooks 是 **effect**，在组件**每次渲染的时候都会执行**
* VCA 是 **setup**，仅在组件初始化的时候**执行一次**
...
* Hooks 移除了生命周期的概念，取而代之的是 [effect](https://reactjs.org/docs/hooks-effect.html)
* VCA 则近乎完整地保留了生命周期概念与函数
